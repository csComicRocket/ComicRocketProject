...
class PageTree:
 ...
 content=None, encodeType=None, pullTS=None):
    ...
    def getPageTreeData(self):
        ...
        """This turns the attributes into a dict and uses json to generate the string."""
        pageTreeDataString = {}
        pageTreeDataString["EncodeType"] = self.getEncodeType(0)
        pageTreeDataString["MimeType"] = self.getMimeType(0)
        pageTreeDataString["AuthorTS"] = self.getAuthorTS(0)
        pageTreeDataString["PullTS"] = self.getPullTS(0)
        if (self.getRevisionNum(0) == None):
            self.setRevisionNum(0, 0)
...
if __name__== "__main__":
    ...
    pt = PageTree("http://www.xkcd.com")
    print pt.getPageTreeData()
